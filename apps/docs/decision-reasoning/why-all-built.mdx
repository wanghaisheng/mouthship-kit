...
```bash
bun turbo dev --filter=@mouthshipkit/web... --filter=!./packages/config/* --filter=!@mouthshipkit/ui --only
```
...
{
  "scripts": {
    "dev:web:api": "bun turbo dev --filter=@mouthshipkit/web... --filter=!./packages/config/* --filter=!@mouthshipkit/ui --only"
  }
}
```
